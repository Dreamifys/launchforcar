# Windows车机模拟器设置脚本 - 支持armeabi-v7a应用
# 适用于Windows系统，使用x86_64镜像（性能最佳）

Write-Host "====================================" -ForegroundColor Cyan
Write-Host "车机模拟器设置 - 支持ARM32应用" -ForegroundColor Cyan
Write-Host "====================================" -ForegroundColor Cyan
Write-Host ""
...
# 安装x86_64系统镜像（推荐Android 13，性能好且兼容性佳）
Write-Host "正在安装系统镜像 (Android 13, x86_64, Google APIs)..." -ForegroundColor Gray
$sdkmanager --sdk_root="$sdkPath" "system-images;android-33;google_apis;x86_64"
...
# 创建车机AVD
Write-Host "正在创建车机模拟器 (CarArm32)..." -ForegroundColor Gray
...
# 修改AVD配置为车机分辨率
$avdPath = "$env:USERPROFILE\.android\avd\CarArm32.avd\config.ini"
...
Write-Host "  emulator -avd CarArm32" -ForegroundColor White
Write-Host ""
Write-Host "安装APK命令:" -ForegroundColor Yellow
Write-Host "  adb install d:\LPOS\9.1.87悬浮共存.apk" -ForegroundColor White
Write-Host ""
Write-Host "如果安装失败（架构不兼容），可以尝试:" -ForegroundColor Yellow
Write-Host "1. 使用第三方模拟器（夜神/雷电/MuMu）" -ForegroundColor Gray
Write-Host "2. 或者下载armeabi-v7a镜像:" -ForegroundColor Gray
Write-Host '   sdkmanager "system-images;android-33;default;armeabi-v7a"' -ForegroundColor Gray
Write-Host ""
